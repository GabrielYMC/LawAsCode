...
| 任務 | 產出 | 狀態 |
|------|------|------|
| 提案服務層 | `$lib/server/proposals/service.ts` — 提案 CRUD、狀態轉移、投票管理 | 完成 |
| 狀態轉移 API | `/api/proposals/[id]/transition` POST — 驗證角色後執行狀態轉移 | 完成 |
| 投票 API | `/api/proposals/[id]/vote` POST/GET — 發起/投票/結束投票 | 完成 |
| 控制台操作按鈕 | Dashboard 按鈕 wired up：直接轉移或發起投票後跳轉至詳情頁 | 完成 |
| 提案詳情頁操作 | 根據角色顯示可執行操作（議長/議員/會長各自看到不同按鈕） | 完成 |
| 即時投票 UI | 進行中投票面板：計票、投票 chip、贊成/反對/棄權按鈕、結束投票 | 完成 |
| 新增提案頁 | `/proposals/new` — 表單：基本資訊 + 動態修正條文（可增刪多條） | 完成 |
| 提案列表整合 | 提案列表加「+ 新增提案」按鈕（僅議員/議長/會長可見） | 完成 |

#### 狀態轉移流程

角色 × 狀態決定可執行操作：
- **秘書長**：PROPOSED → 排入議程（直接轉移）
- **議長**：FIRST_READING → 交付委員會（直接） / 逕付二讀（需投票） / 不予審議（需投票）
- **議員**：COMMITTEE → 審查完成（需投票）
- **議長**：SECOND_READING → 二讀通過（需投票） / 重付審查（需投票）
- **議長**：THIRD_READING → 三讀通過（需投票）
- **會長**：PENDING_PROMULGATION → 公布施行（直接轉移）

#### 投票流程

1. 有權角色點擊「發起表決」→ 建立 VoteSession（status: open）
2. 議員在提案詳情頁投票（贊成/反對/棄權），一人一票
3. 議長點擊「結束投票並計算結果」→ 計算門檻 → 若通過自動推進狀態

#### 測試驗證

以實際操作驗證完整流程：
1. 議長在控制台點擊「交付委員會」→ proposal-002 從一讀推進至委員會 ✅
2. 議員發起「審查完成」表決 → 投票面板出現 ✅
3. 議員投贊成票 → 計票更新、防重複投票 ✅
4. 議長結束投票 → 表決通過、自動推進至二讀 ✅
5. 新增提案頁面表單驗證與提交 ✅
...
| 任務 | 產出 | 狀態 |
|------|------|------|
| 功能說明文件 | `README.md` — 功能總覽、技術架構、快速開始、部署指南、fork 指南 | 完成 |
| 公開法規瀏覽器 | `/laws` 不需登入即可存取（原已公開） | 確認 |
| 公開 AI 顧問 | `/advisor` 移除登入限制，所有人可使用 | 完成 |
| MIT License | `LICENSE` — MIT 授權 | 完成 |
| PLAN.md 更新 | Phase 4 三項 checkbox 勾選完成 | 完成 |

#### 公開存取變更

- `advisor/+page.server.ts`：移除 `requireAuth` 守衛
- `api/advisor/+server.ts`：移除登入檢查
- `guards.ts`：`/advisor` 加入 `'public'` 權限
- `+layout.svelte`：AI 顧問 nav item 改為 `auth: false`
...
|------|------|------|
| 首頁 | `/` | 法規統計、功能入口 |
| 法規瀏覽 | `/laws`, `/laws/[slug]` | 22 部法規完整瀏覽 |
| AI 顧問 | `/advisor` | 三模式法規諮詢 |
| 全文搜尋 | `/search` | fuse.js 模糊搜尋 |
| 登入 | `/login` | 開發模式角色選擇 |
